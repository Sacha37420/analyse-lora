"""
Settings pour Analyse LoRa.
Les variables sensibles sont lues depuis .env via python-decouple.
"""
import os
from decouple import config

SECRET_KEY = config('SECRET_KEY', default='django-insecure-analyse-lora-change-in-production')
...
_DB_SCHEMA = config('DB_SCHEMA', default='analyse_lora')
...
REST_FRAMEWORK = {
    ...
    'DEFAULT_AUTHENTICATION_CLASSES': ['api.authentication.KeycloakJWTAuthentication'],
    'DEFAULT_PERMISSION_CLASSES':     ['rest_framework.permissions.IsAuthenticated'],
}

KEYCLOAK_ISSUER_URI = config('KEYCLOAK_ISSUER_URI', default='http://keycloak:8080/realms/ssolab')
KEYCLOAK_CLIENT_ID  = config('KEYCLOAK_CLIENT_ID',  default='analyse-lora')
...
SPECTACULAR_SETTINGS = {
    'TITLE':                   'Analyse LoRa API',
    'DESCRIPTION':             'Documentation interactive — capteurs LoRa, données et grandeurs calculées',
    'VERSION':                 '1.0.0',
    ...
    'POSTPROCESSING_HOOKS': ['config.spectacular_hooks.add_bearer_security'],
}
